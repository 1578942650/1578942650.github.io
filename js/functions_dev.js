...
$(function () {
	...
	$("#content").css("margin-bottom", Math.max(($window.height() - $("#content").height()) / 2, 10));
	$("#content").css("margin-left", Math.max(($window.width() - $("#content").width()) / 2, 10));
	//set up the rest div
	$("#secondpageDiv").css("height", $window.height());
	$("#thirdpageDiv").css("height", $window.height());
...
function timeElapse(date, mode) {
	...
	if (mode == 1) {
		...
		if (days < 0) {
			days += getDaysInMonth(current.getMonth());//这里没问题是因为month0是1月 数组里第0个也是对应1月
			current.setDate(current.getDate() - 1);
		}
